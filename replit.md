...
Recent Changes:
- July 22, 2025: ✅ COMPLETED full serverless migration and bug fixes. Fixed three critical issues: (1) Created separate Netlify functions for auth endpoints (auth-login, auth-register, auth-user) to resolve "Endpoint not found" errors, (2) Updated inquiry modal to use correct /api/contact endpoint instead of franchise-specific endpoints, (3) Added unique images for each franchise and business listing to replace duplicate images. Updated frontend API client to properly route to correct Netlify functions in production. Database seeded with diverse franchises (MILKSTER, TechFlow, Fitness Plus, Clean Pro) and businesses (Coffee House, Tech Startup, FitZone Gym, Mountain Bakery) each with distinct images. All authentication, inquiry submission, and admin panel functionality now working in serverless environment.
